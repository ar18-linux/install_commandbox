...
echo "${ar18_sudo_password}" | sudo -S -k rm -rf "/tmp/linux-jre64"

cd /tmp
wget https://www.ortussolutions.com/parent/download/commandbox/type/linux-jre64

echo "${ar18_sudo_password}" | sudo -S -k rm -rf "${install_dir}/commandbox"

echo "${ar18_sudo_password}" | sudo -S -k unzip "/tmp/linux-jre64" -d "${install_dir}/commandbox"
